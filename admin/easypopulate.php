...
<td class="main" valign="top">
  ...
  <p class="main"><?php echo TEXT_INTRO ?></p>
  <p class="warning"><?php echo TEXT_WARNING ?></p>
  <ol>
  	<li>
		<h2><?php echo TEXT_HEADING_STEP1 ?></h2>
		<p><?php echo TEXT_INFO_STEP1 ?></p>
		<?php echo tep_draw_form('export','easypopulate.php?download=stream&dltype=full'); ?>
		<input type="submit" value="<?php echo TEXT_HEADING_STEP1 ?>" />
		</form>
	</li>
	<li>
		<h2><?php echo TEXT_HEADING_STEP2 ?></h2>
		<p><?php echo TEXT_INFO_STEP2 ?></p>
	</li>
	<li>
		<h2><?php echo TEXT_HEADING_STEP3 ?></h2>
		<p><?php echo TEXT_INFO_STEP3 ?></p>
	</li>	
	<li>
		<h2><?php echo TEXT_HEADING_STEP4 ?></h2>
		<FORM ENCTYPE="multipart/form-data" ACTION="easypopulate.php?split=0" METHOD=POST>		  
			<INPUT TYPE="hidden" name="MAX_FILE_SIZE" value="100000000">
			<input name="usrfl" type="file" size="50">
			<input type="submit" name="buttoninsert" value="<?php echo TEXT_HEADING_STEP4 ?>">
		</form>
		<p><?php echo TEXT_INFO_STEP4 ?></p>
	</li>
...
if ($localfile or (is_uploaded_file($usrfl) && $split==0)) {
	...
	if ($usrfl){
		...
		echo '<p class="smallText" style="margin-left: 40px; color: #7187BB;">';
		echo "Importdatei <em>$usrfl_name</em> wurde hochgeladen. <br>";
		echo "Tempor&auml;rer Dateiname: " . $usrfl . "<br>";
		echo "Gr&ouml;sse: " . round($usrfl_size/1024,2) . " kb</p>";
		echo '<div class="smallText" style="white-space: nowrap; width: 750px; height: 300px; overflow: auto; margin-left: 40px; border: 1px solid silver; "><p>';
...
function ep_create_filelayout($dltype){
	...
	case 'full':
		...
		$filelayout = array(
			...
			'v_products_image2'	=> $iii++,
			'v_products_image3'	=> $iii++,
			'v_products_image4'	=> $iii++,
			);
    ...
    // VJ product attribs begin
     $header_array = array(
		...
		$filelayout_sql = "SELECT
			...
			p.products_image2 as v_products_image2,
			p.products_image3 as v_products_image3,
			p.products_image4 as v_products_image4,
	...
	if ($v_products_model != "") {
		...
		if (tep_db_num_rows($result) == 0)  {
			...
			*/
			...
			// additional images for swisscart
			$query .= " 	'$v_products_image2',
							'$v_products_image3',
							'$v_products_image4',";
...
*/
			...
			// additional images for swisscart
			$query .= '" ,products_image2="'.$v_products_image2.
					  '" ,products_image3="'.$v_products_image3.
					  '" ,products_image4="'.$v_products_image4;
