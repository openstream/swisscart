...
// Generate product list based on chosen category or search keywords
	...
	if (isset($_POST['search'])) {
		...
		$products_query = tep_db_query("select distinct p.products_id, p.products_price, p.products_model, pd.products_name from (" . TABLE_PRODUCTS . " p left join ". TABLE_PRODUCTS_TO_CATEGORIES. " p2c using (products_id) left join ". TABLE_CATEGORIES ." c using (categories_id)) left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on (p.products_id = pd.products_id) where c.categories_status=1 and p.products_status = '1' and pd.language_id = '" . $languages_id . "' and (" . $product_search . ") order by pd.products_name");
		$not_found = ((tep_db_num_rows($products_query)) ? false : true);
	}

	if (!isset($_POST['search'])) {
		$product_search = " where c.categories_status=1 and p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and p.products_id = p2c.products_id and p2c.categories_id = c.categories_id ";
