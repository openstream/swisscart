...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
	...
	function tep_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false) {
		...
		if ($include_itself) {
			$category_query = tep_db_query('select cd.categories_name from ' . TABLE_CATEGORIES .' c left join '. TABLE_CATEGORIES_DESCRIPTION . ' cd using (categories_id) where cd.language_id = "' . (int)$languages_id . '" and cd.categories_id = "' . (int)$parent_id . '"');
	...
	function tep_draw_products_pull_down($name, $parameters = '', $exclude = '') {
		...
		$products_query = tep_db_query("select distinct p.products_id, c.categories_status, p.products_status , pd.products_name, p.products_price from " . TABLE_PRODUCTS . " p left join ". TABLE_PRODUCTS_TO_CATEGORIES. " p2c using (products_id) left join ". TABLE_CATEGORIES ." c using (categories_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by products_name");
		while ($products = tep_db_fetch_array($products_query)) {
			if (!in_array($products['products_id'], $exclude)) {
				if($products['categories_status']==1 && $products['products_status']==1){
					$status_marking ='&nbsp;';
				} else {
					$status_marking ='inactive: ';
				}

				$select_string .= '<option value="' . $products['products_id'] . '">' .  $status_marking. ' '.$products['products_name'] . ' (' . $currencies->format($products['products_price']) . ')</option>';
			}
		}
...
// TABLES: products, products_to_categories, categories
	function tep_products_in_category_count($categories_id, $include_deactivated = false) {
		...
		if ($include_deactivated) {
			...
			$products_query = tep_db_query("select count(*) as total from (" . TABLE_PRODUCTS . " p left join ". TABLE_PRODUCTS_TO_CATEGORIES. " p2c using (products_id) left join ". TABLE_CATEGORIES ." c using (categories_id)) where c.categories_status=1 and p.products_status = '1' and p2c.categories_id = '" . (int)$categories_id . "'");
		}
		...
		if ($include_deactivated) {
			...
			$childs_query = tep_db_query("select categories_id from " . TABLE_CATEGORIES . " where categories_status=1 and parent_id = '" . (int)$categories_id . "'");
		}
...
// Sets the status of a banner
	function tep_set_banner_status($banners_id, $status) {
		if ($status == '1') {
			...
			return tep_db_query("update " . TABLE_BANNERS . " set status = '0', date_status_change = '". date('Y-m-d H:i:s') ."' where banners_id = '" . $banners_id . "'");
		} else {
			return -1;
		}
	}
...
// Sets the status of a product
	function tep_set_product_status($products_id, $status) {
		if ($status == '1') {
			return tep_db_query("update " . TABLE_PRODUCTS . " set products_status = '1', products_last_modified = '". date('Y-m-d H:i:s') ."' where products_id = '" . (int)$products_id . "'");
		} elseif ($status == '0') {
			return tep_db_query("update " . TABLE_PRODUCTS . " set products_status = '0', products_last_modified = '". date('Y-m-d H:i:s') ."' where products_id = '" . (int)$products_id . "'");
		} else {
			return -1;
		}
	}
...
// Sets the status of a category and all nested categories and products whithin.
	function tep_set_categories_status($category_id, $status) {
		if ($status == '1') {
			tep_db_query("update " . TABLE_CATEGORIES . " set categories_status = '1', last_modified = '". date('Y-m-d H:i:s') ."' where categories_id = '" . $category_id . "'");
			$tree = tep_get_category_tree($category_id);
			for ($i=1; $i<sizeof($tree); $i++) {
				tep_db_query("update " . TABLE_CATEGORIES . " set categories_status = '1', last_modified = '". date('Y-m-d H:i:s') ."' where categories_id = '" . $tree[$i]['id'] . "'");
			}
		} elseif ($status == '0') {
			tep_db_query("update " . TABLE_CATEGORIES . " set categories_status = '0', last_modified = '". date('Y-m-d H:i:s') ."' where categories_id = '" . $category_id . "'");
			$tree = tep_get_category_tree($category_id);
			for ($i=1; $i<sizeof($tree); $i++) {
				tep_db_query("update " . TABLE_CATEGORIES . " set categories_status = '0', last_modified = '". date('Y-m-d H:i:s') ."' where categories_id = '" . $tree[$i]['id'] . "'");
			}
		}
	}
...
// Sets the status of a product on special
	function tep_set_specials_status($specials_id, $status) {
		if ($status == '1') {
			...
			return tep_db_query("update " . TABLE_SPECIALS . " set status = '0', date_status_change = '". date('Y-m-d H:i:s') ."' where specials_id = '" . (int)$specials_id . "'");
		} else {
			return -1;
		}
	}
...
// Sets the sort order of a product
	function tep_set_product_sort_order($products_id, $sort_order) {
		return tep_db_query("update " . TABLE_PRODUCTS . " set products_sort_order = '" . $sort_order . "', products_last_modified = '". date('Y-m-d H:i:s') ."' where products_id = '" . (int)$products_id . "'");
	}
?>
