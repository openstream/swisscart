mox.require([
	...
	mox.create('static mox.manager.FileManagerDefaultTheme', {
		...
		onError : function(m, e, r, co, text) {
			...
			if (typeof(e) == 'object') {
				if (e.level == 'AUTH') {
					if (r.login_url.indexOf('return_url') != -1)
						doc.location = r.login_url;
					else if (r.login_url.indexOf('://') == -1)
		...
		preview : function(p, t, nv) {
			...
			if (!nv && t != 'zip')
				DOM.setAttrib('preview', 'src', '../../stream/' + mox.defaultDoc + '?cmd=fm.streamFile&path=' + encodeURIComponent(p));
			else
				DOM.setAttrib('preview', 'src', 'blank.html');
		...
		insert : function() {
			...
			var win = opener || parent, man;

			if (!sr || sr.type == 'folder' || DOM.hasClass('insert', 'disabled'))
				return;

			if (win && win != window) {
				man = win.mcFileManager || win.mcImageManager;
				...
				if (man) {
					...
					mcFileManager.execRPC('fm.insertFiles', {path0 : sr.path},  function (data) {
						...
						WindowManager.hideProgress();
						mcFileManager.restoreSelection();
		...
		download : function(p) {
			...
			ifr.src = '../../stream/' + mox.defaultDoc + '?cmd=fm.download&path=' + p;
		},
		...
		view : function(p) {
			...
			window.open('../../stream/' + mox.defaultDoc + '?cmd=fm.streamFile&path=' + encodeURIComponent(p), 'View');
		},
