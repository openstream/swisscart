...
// set php_self in the local scope
	...
	define('CURRENT_PAGE', basename($PHP_SELF)); // allows use anywhere in the framework (Claudio)
...
// calculate category path
	...
	if (tep_not_null($cPath)) {
		...
		do{
			...
			$query = tep_db_query('select categories_status from '. TABLE_CATEGORIES .' where categories_id="'.$current_category_id.'"');
			...
			$category_moved = true;
		} while ((int)$current_category_id != 0);
		if ($category_moved) tep_redirect(tep_href_link(CURRENT_PAGE, tep_get_all_get_params(array('cPath')).($cPath?'cPath='.$cPath:'')) );
	} else {
