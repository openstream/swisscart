...
// insert current cart contents in database
			if (is_array($this->contents)) {
				...
				while (list($products_id, ) = each($this->contents)) {
					// check availability (categories enable/disable)
					$check=tep_db_fetch_array(tep_db_query('select count(*) as total from '. TABLE_PRODUCTS . ' p left join '.TABLE_PRODUCTS_TO_CATEGORIES . ' p2c using(products_id) left join '. TABLE_CATEGORIES .' c using (categories_id) where c.categories_status=1 and p.products_status=1 and p.products_id="'. (int)$products_id .'"'));
					if($check['total']==0){
