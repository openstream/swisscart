...
// TABLES: products, products_to_categories, categories
	function tep_count_products_in_category($category_id, $include_inactive = false) {
		...
		if ($include_inactive == true) {
			...
			$products_query = tep_db_query("select count(*) as total from (" . TABLE_PRODUCTS . " p left join ". TABLE_PRODUCTS_TO_CATEGORIES. " p2c using (products_id) left join ". TABLE_CATEGORIES ." c using (categories_id)) where c.categories_status=1 and p.products_status = '1' and p2c.categories_id = '" . (int)$category_id . "'");
		}
		$products = tep_db_fetch_array($products_query);
		$products_count += $products['total'];

		$child_categories_query = tep_db_query("select categories_id from " . TABLE_CATEGORIES . " where categories_status=1 and parent_id = '" . (int)$category_id . "'");
...
// TABLES: categories
	function tep_has_category_subcategories($category_id) {
		$child_category_query = tep_db_query("select count(*) as count from " . TABLE_CATEGORIES . " where categories_status=1 and parent_id = '" . (int)$category_id . "'");
...
// TABLES: customers, address_book
	...
	function tep_get_categories($categories_array = '', $parent_id = '0', $indent = '') {
		...
		$categories_query = tep_db_query("select c.categories_id, cd.categories_name from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_status=1 and parent_id = '" . (int)$parent_id . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' order by sort_order, cd.categories_name");
...
// Return table heading with sorting capabilities
	function tep_create_sort_heading($sortby, $colnum, $heading) {
		...
		if (strpos($sort_prefix, '&') !== false) {
		$sort_prefix = str_replace('&', '&amp;', $sort_prefix);
		}
...
// TABLES: categories
	function tep_get_parent_categories(&$categories, $categories_id) {
		$parent_categories_query = tep_db_query("select parent_id from " . TABLE_CATEGORIES . " where categories_status=1 and categories_id = '" . (int)$categories_id . "'");
...
// TABLES: products_to_categories
	function tep_get_product_path($products_id) {
		...
		$category_query = tep_db_query("select p2c.categories_id from " . TABLE_PRODUCTS . " p left join ". TABLE_PRODUCTS_TO_CATEGORIES. " p2c using (products_id) left join ". TABLE_CATEGORIES ." c using (categories_id) where p.products_id = '" . (int)$products_id . "' and c.categories_status=1 and p.products_status = '1' limit 1");
...
// TABLES: products_description
	function sc_get_products_teaser($product_id, $language = '', $length = PRODUCT_LIST_TEASER_LENGTH) {
		global $languages_id;
	...
	$teaser = '<br>' . substr($product_description['products_description'], 0, $length);
	if(strlen($product_description['products_description']) > 0) $teaser .= ' ...';
	return $teaser;
	}
