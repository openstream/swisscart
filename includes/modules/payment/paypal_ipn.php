...
/*
  ...
  Changelog
  2006-10-12: Umlautkonvertierung wegen PayPal-Bug
*/
...
// class constructor
    function paypal_ipn() {
      ...
      $this->identifier = 'swisscart PayPal IPN v1.0';
...
// class methods
    ...
    function process_button() {
      ...
      if (!in_array($my_currency, array('CHF','CAD', 'EUR', 'GBP', 'JPY', 'USD'))) {
        $my_currency = 'CHF';
      }
...
// third attribute onwards will not be shown at PayPal
	  ...
	  $parameters['first_name'] = $this->convert_umlauts($order->billing['firstname']);
	  $parameters['last_name'] = $this->convert_umlauts($order->billing['lastname']);
	  $parameters['address1'] = $this->convert_umlauts($order->billing['street_address']);
	  $parameters['address2'] = $this->convert_umlauts($order->billing['suburb']);
	  $parameters['city'] = $this->convert_umlauts($order->billing['city']);
	  ...
	  $parameters['state'] = $this->convert_umlauts($order->billing['state']);
	  $parameters['zip'] = $order->billing['postcode'];
	  $parameters['country'] = $this->convert_umlauts($order->billing['country']['iso_code_2']);
...
// unregister session variables used during checkout
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Transaction Currency', 'MODULE_PAYMENT_PAYPAL_IPN_CURRENCY', 'Selected Currency', 'The currency to use for transactions', '6', '6', 'tep_cfg_select_option(array(\'Selected Currency\',\'Only CHF\',\'Only USD\',\'Only CAD\',\'Only EUR\',\'Only GBP\',\'Only JPY\'), ', now())");
	...
	// PayPal Bug bei Umlauten in POST-Variablen. Sollte gemäss Tech Support im nächsten Versions-Update 
	// im September 2006 behoben werden. Jetzt ist aber schon Oktober ...
	function convert_umlauts ($val) {
