<?php
/*
  $Id: pdf_datasheet_functions v1.30 2003/05/11 13:46:29 ip chilipepper.it Exp $
...
require(DIR_WS_INCLUDES . 'pdf/pdf_datasheet_config.php');
require(DIR_WS_INCLUDES . 'pdf/fpdf/fpdf.php');
...
var $y0;
 ...
 global $languages_id;
	...
	if(SHOW_LOGO != 0 )
    	{
    		// Store Logo
    		$this->Image(PDF_STORE_LOGO,'20','18','36','', PDF_STORE_LOGO_TYPE, FILENAME_DEFAULT);
	} else {
    		//Store Name
			$store_name = $_SERVER['HTTP_HOST'];
			$this->SetFont('helvetica','B',18);
			$this->SetLineWidth(0);
    		$w=$this->GetStringWidth($store_name);
 ...
 function Footer()
 {
	...
	$this->Cell(0,6,TEXT_PDF_FOOTER . $_SERVER['HTTP_HOST'] . ', ' . STORE_OWNER_EMAIL_ADDRESS . '                       ' . TEXT_PAGE . ' ' . $this->PageNo().'/{nb}',0,0,'L',1,'mailto:'. STORE_OWNER_EMAIL_ADDRESS);
 }
 ...
 if(SHOW_IMAGES && strlen($data_array[10]))
	...
	else
	{
     ...
     if(SHOW_PRICE)
     {
		$this->SetTextColor(BODY_COLOR_TEXT);
		...
		//echo '<pre>'; print_r($desc); die();
	 	$x=$this->GetX();
				foreach ($desc as $desc_row){
					...
					// set flag if unordered list item
					$li = (strstr($desc_row, "<li>") == false && strstr($desc_row, "<LI>") == false) ? false : true;
					//echo strstr($desc_row, "<LI>");
					//die();
					/*if($li == true) {
						echo $desc_row;
						die();
					...
					if($li == true) {
						//$column_width = ($this->w-30)/2;
						$column_width = $this->w-30;
						$desc_row = trim($desc_row);
						$this->MultiCellBlt($column_width, 6, chr(149), $desc_row);
						...
						if($found==true){
							...
							$this->SetX($x);
							$this->MultiCell(180-$this->rMargin,4,$text2,0,'L');
						}
  ...
  function ProductsDataSheet($languages_id, $products_id){
	  ...
	  $imageheight=SMALL_IMAGE_HEIGHT*PDF_TO_MM_FACTOR;
	    ...
	    $print_catalog_query = tep_db_query("select p.products_id, pd.products_name, pd.products_description, p.". PDF_ALT_IMAGE .", p.products_model, p.products_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.products_status, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price,  s.expires_date, m.manufacturers_name
                               from " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on p.manufacturers_id = m.manufacturers_id, " . TABLE_PRODUCTS_DESCRIPTION . " pd left join " . TABLE_SPECIALS . " s on pd.products_id = s.products_id where
		...
		if (OPTIONS_AS_IMAGES_ENABLED) 
		{
			...
			// Option values are displayed horizontaly
			{
                while ($products_options_name_values = tep_db_fetch_array($products_options_name_query)) {
                    ...
                    $largest_y = $this->GetY();
                    //OAI query
                    $products_options_query = tep_db_query("select pov.products_options_values_id, pov.products_options_values_name, pa.options_values_price, pa.price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov where pa.products_id = '" . (int)$products_id . "' and pa.options_id = '" . (int)$products_options_name_values['products_options_id'] . "' and pa.options_values_id = pov.products_options_values_id and pov.language_id = '" . (int)$languages_id . "'");
...
function MultiCellBlt($w, $h, $blt, $txt, $border=0, $align='L', $fill=0)
{
	...
	//Output bullet
	$this->Cell($blt_width, $h, $blt, $border, '', $fill);
	//Output text
	$this->MultiCell($w-$blt_width, $h, $txt, $border, $align, 0);
