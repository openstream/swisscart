...
<?php
	if ($category_depth == 'nested') {
		$category_query = tep_db_query("select cd.categories_name, cd.categories_description, c.categories_image from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_status=1 and c.categories_id = '" . (int)$current_category_id . "' and cd.categories_id = '" . (int)$current_category_id . "' and cd.language_id = '" . (int)$languages_id . "'");
...
// We are asked to show only a specific category
				$listing_sql = 'select distinct p.products_id,' . $select_column_list . ' p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from (' . TABLE_PRODUCTS . ' p left join '.TABLE_PRODUCTS_TO_CATEGORIES .' p2c on p2c.products_id=p.products_id left join '. TABLE_CATEGORIES . ' c on c.categories_id=p2c.categories_id) left join ' . TABLE_SPECIALS . ' s on p.products_id = s.products_id, ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_MANUFACTURERS . ' m where c.categories_status=1 and p.products_status = "1" and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = "' . (int)$HTTP_GET_VARS["manufacturers_id"] . '" and pd.products_id = p2c.products_id and pd.language_id = "' . (int)$languages_id . '" and p2c.categories_id = "' . (int)$HTTP_GET_VARS["filter_id"] . '"';
			} else {
// We show them all
				$listing_sql = 'select distinct p.products_id,' . $select_column_list . ' p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from (' . TABLE_PRODUCTS . ' p left join '.TABLE_PRODUCTS_TO_CATEGORIES .' p2c on p2c.products_id=p.products_id left join '. TABLE_CATEGORIES . ' c on c.categories_id=p2c.categories_id) left join ' . TABLE_SPECIALS . ' s on p.products_id = s.products_id, ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_MANUFACTURERS . ' m where c.categories_status=1 and p.products_status = "1" and pd.products_id = p.products_id and pd.language_id = "' . (int)$languages_id . '" and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = "' . (int)$HTTP_GET_VARS["manufacturers_id"] . '"';
			}
		} else {
// show the products in a given categorie
			if (isset($HTTP_GET_VARS["filter_id"]) && tep_not_null($HTTP_GET_VARS["filter_id"])) {
// We are asked to show only specific catgeory
				$listing_sql = 'select ' . $select_column_list . ' p.products_id, p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from (' . TABLE_PRODUCTS . ' p left join '.TABLE_PRODUCTS_TO_CATEGORIES .' p2c on p2c.products_id=p.products_id left join '. TABLE_CATEGORIES . ' c on c.categories_id=p2c.categories_id) left join ' . TABLE_SPECIALS . ' s on p.products_id = s.products_id, ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, ' . TABLE_MANUFACTURERS . ' m where c.categories_status=1 and p.products_status = "1" and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = "' . (int)$HTTP_GET_VARS["filter_id"] . '" and pd.products_id=p.products_id and pd.language_id = "' . (int)$languages_id . '" and p2c.categories_id = "' . (int)$current_category_id . '"';
			} else {
// We show them all
				$listing_sql = 'select ' . $select_column_list . ' p.products_id, p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from ' . TABLE_PRODUCTS_DESCRIPTION . ' pd, (' . TABLE_PRODUCTS . ' p left join '.TABLE_PRODUCTS_TO_CATEGORIES .' p2c on p2c.products_id=p.products_id left join '. TABLE_CATEGORIES . ' c on c.categories_id=p2c.categories_id) left join ' . TABLE_MANUFACTURERS . ' m on p.manufacturers_id = m.manufacturers_id left join ' . TABLE_SPECIALS . ' s on p.products_id = s.products_id where c.categories_status=1 and p.products_status = "1" and pd.products_id = p2c.products_id and pd.language_id = "' . (int)$languages_id . '" and p2c.categories_id = "' . (int)$current_category_id . '"';
			}
		}
...
?>
		<td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0">
			<tr>
				...
				$category_query = tep_db_query("select cd.categories_name, cd.categories_description from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_status=1 and c.categories_id = '" . (int)$current_category_id . "' and cd.categories_id = '" . (int)$current_category_id . "' and cd.language_id = '" . (int)$languages_id . "'");
				$category = tep_db_fetch_array($category_query);
...
// optional Product List Filter
		if (PRODUCT_LIST_FILTER > 0) {
			if (isset($HTTP_GET_VARS['manufacturers_id'])) {
				...
				$filterlist_sql= 'select distinct m.manufacturers_id as id, m.manufacturers_name as name from ' . TABLE_PRODUCTS . ' p left join '.TABLE_PRODUCTS_TO_CATEGORIES .' p2c on p2c.products_id=p.products_id left join '. TABLE_CATEGORIES . ' c on c.categories_id=p2c.categories_id, ' . TABLE_MANUFACTURERS . ' m where c.categories_status=1 and p.products_status = "1" and p.manufacturers_id = m.manufacturers_id and p2c.categories_id = "' . (int)$current_category_id . '" order by m.manufacturers_name';
			}
...
?>
						<td align="right"><?php if (HEADING_IMAGE_ENABLED == 'true') echo tep_image(DIR_WS_IMAGES . $image, HEADING_TITLE, HEADING_IMAGE_WIDTH, HEADING_IMAGE_HEIGHT); ?></td>
					</tr>
					<tr><td><?php
						if (isset($HTTP_GET_VARS['manufacturers_id'])) {
