...
*/
	...
	$products_query = tep_db_query("select distinct pd.products_name, p.products_image, p.products_image2, p.products_image3, p.products_image4 from (" . TABLE_PRODUCTS . " p left join ". TABLE_PRODUCTS_TO_CATEGORIES. " p2c using (products_id) left join ". TABLE_CATEGORIES ." c using (categories_id)) left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id where c.categories_status=1 and p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and pd.language_id = '" . (int)$languages_id . "'");
