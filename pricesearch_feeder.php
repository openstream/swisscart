...
*/
	...
	$included_categories_query = tep_db_query("SELECT c.categories_id, c.parent_id, cd.categories_name FROM " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd WHERE c.categories_status=1 and c.categories_id = cd.categories_id AND cd.language_id = '". (int)$languages_id. "'");
	$inc_cat = array();
	...
	if ($rows>0) {
		...
		$products_query = tep_db_query("select distinct p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, IF(s.status, s.specials_new_products_price, NULL) as specials_price, p.products_tax_class_id, pd.products_name, pd.products_description, p2c.categories_id FROM (" . TABLE_PRODUCTS . " p left join ". TABLE_PRODUCTS_TO_CATEGORIES. " p2c using (products_id) left join ". TABLE_CATEGORIES ." c using (categories_id)) left join " . TABLE_SPECIALS . " s using (products_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd WHERE c.categories_status=1 and p.products_status = 1 AND pd.products_id = p.products_id AND p.products_price != 0 AND pd.language_id = '" . (int)$languages_id . "' group by p.products_id order by p2c.categories_id, pd.products_name" . $sql_limit);

//$products_query = tep_db_query("select distinct p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_tax_class_id, pd.products_name, pc.categories_id, cd.categories_name FROM " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " pc, " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd WHERE p.products_id = pd.products_id AND p.products_id = pc.products_id AND c.categories_id = pc.categories_id AND c.categories_id = cd.categories_id AND p.products_status = 1 AND p.products_compare_status = 1 AND pd.language_id = '" . (int)$languages_id . "' AND cd.language_id = '" . (int)$languages_id . "' order by c.sort_order, pd.products_name limit $offset, $rows");
